...
CUD_MESSAGES = [
    ...
    "🔺पिल्लै TᴜJSʜᴇ ᴍâRᴇɴɢᴇ ʏâʜɪ ᴅᴇʟʜɪ ᴍâYᴜR ᴠɪʜâR ᴍᴇ ᴊâB ᴍâRᴇɴɢᴇ ᴅᴇᴋʜ ʟᴇɴᱟ 🔥>💀",
...
# Setup Control Bot Handlers with Interactive Setup / Buttons & Password protection
def setup_control_bot_handlers():
    ...
    async def control_start(event):
        buttons = [
            ...
            [Button.inline("📂 See Session / Run with Old", data="setup_check_session")]
        ]
        await event.reply(
            "⚡ **XNS Main Control Panel**\n\n"
            "Welcome! Please choose an option below to start:",
            buttons=buttons
        )
    ...
    async def control_callback(event):
        ...
        if data == "setup_new":
            ...
            await event.edit(
                "🔐 **Security Check:**\n\n"
                "Please enter the bot setup password (default is `xns`):",
                buttons=None
            )
            ...
            if sessions:
                buttons = [
                    [Button.inline("▶️ Run with Old Session", data="run_old")],
                    [Button.inline("🗑️ Clear Old Session & Setup New", data="setup_new")]
                ]
                await event.edit(
                    f"📂 **Sessions Found:** `{len(sessions)}` file(s) exist in `sessions/` directory.\n\n"
                    "Would you like to run with the existing session or clear them?",
                    buttons=buttons
                )
            else:
                buttons = [[Button.inline("🚀 Start New Setup", data="setup_new")]]
                await event.edit(
                    "ℹ️ **No active sessions found.** Please start a new setup.",
                    buttons=buttons
                )
            ...
            await event.respond("✅ **System running successfully with old sessions!** Type `/bots` or `/status` to check.")

        elif data.startswith("dl_"):
            # Handled by song bot worker if applicable
            pass
    ...
    async def control_interactive_handler(event):
        ...
        if sender_id not in SETUP_STATES:
            # Handle standard control commands if user is already set up
        ...
        if step == "WAITING_PASSWORD":
            ...
            if val == correct_pwd:
                ...
                await event.reply(
                    "✅ **Password Verified!**\n\n"
                    "👉 Please enter your **Admin Chat ID** (Telegram numeric ID) to use the userbot as owner:"
                )
            else:
            ...
            if val.lstrip("-").isdigit():
                ...
                await event.reply(
                    "✅ **Owner ID saved!**\n\n"
                    "👉 Now, please enter your **Primary Phone Number** (with country code, e.g., `+919876543210`):"
                )
            else:
...
async def main():
    ...
    print("🚀 Initializing Permanent XNS Control Bot with Interactive Setup...")
