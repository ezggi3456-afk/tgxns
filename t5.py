...
async def play_replied_audio(event, phone, reply):
    ...
    # Explicitly join call first to guarantee connection before playing audio
    try:
    ...
    status = await event.reply("⬇️ **Downloading audio file & connecting call...**")
    ...
    try:
        ...
        if duration_secs <= 0:
            try:
                ...
                duration_secs = 14.0  # Silent safe fallback without spamming logs
        ...
        async def exact_audio_loop_worker():
            while chat_id in ACTIVE_VOICE_LOOPS and ACTIVE_VOICE_LOOPS[chat_id].get("active"):
                try:
                    ...
                    # Suppress ffprobe missing traceback spam in production logs
                    if "ffprobe not installed" not in str(e):
